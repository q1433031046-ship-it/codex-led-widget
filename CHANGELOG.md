...
- 降低后台渲染开销：窗口收起或隐藏时暂停动画和图表重绘；额度保持每 60 秒刷新，本机模型用量扫描改为每 5 分钟，并在查看时按缓存时效请求更新。
- 统计页新增额度历史周期列表，复用按账号和额度来源隔离的本地历史；补上旧版历史兼容与原子写入，跨周期和重启后可继续查看已保留的记录。
- 修复多显示器不同 DPI、负坐标和接缝区域下的磁吸边界；收起时只保留当前屏幕内的窄边，避免邻屏看到完整窗口。
- 修复磁吸展开时内容晚于窗口出现的问题：展开前预热 Renderer，并按动画帧同步原生可见区域，让卡片与窗口同时滑入。
